...
| Live capture by the owner, 2026-09-22 (task T0.3, recorded with `--record`; reduced, anonymised excerpt in `testdata/live-2026-09-22.jsonl`) | game build of 2026-09-22 | — | 45 raw frames over 6.5 min: connect, three ticks, rename, pause, minimise, main menu, reload |
...
**[live]** when they were measured on the owner's raw live capture of
...
evidence from the real game (task T0.3, record mode).
...
(the owner's profile name in the live run; `Player` in the anonymised fixture). **[live]**
...
Owner's run: save already loaded at connect; Latium shown; an island renamed
...
- The format is a flat little-endian binary layout with length prefixes —
  ...
  C#/.NET plan B (input for task T0.4).
