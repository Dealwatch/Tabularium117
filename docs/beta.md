...
3. `tabularium117.exe` starten. Windows zeigt beim ersten Start
   ...
   **„Weitere Informationen"** und dann **„Trotzdem ausführen"** klicken. Das
   liegt an der fehlenden kommerziellen Codesignatur, nicht an einem
   Sicherheitsproblem; siehe README.
...
- die Log-Ausgabe mit `--verbose` (Tabularium 117 mit diesem Flag neu
  starten und den Fehler reproduzieren);
...
3. Start `tabularium117.exe`. Windows shows "Windows protected your PC"
   ...
   anyway"**. That is because the binary is not commercially code-signed,
   not a security problem; see the README.
