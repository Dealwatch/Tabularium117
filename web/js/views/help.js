...
// notification opt-ins of task T6.3 and the "keep the screen on" opt-in of
// task T7.4.
//
...
// buildScreenSettings is the phone opt-in of task T7.4: a second screen that
...
function buildScreenSettings() {
  ...
  const note = document.createElement("p");
  note.className = "muted";
  note.textContent = wakeLock.supported() ? i18n.t("wakeLockExplain") : i18n.t("wakeLockUnsupported");
  section.append(note);
  return section;
}
