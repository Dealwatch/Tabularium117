...
export async function renderHistory(container, islandId, guid, store) {
  ...
  let data = null;
  let lastLoadAt = 0;
  // A range switched twice in a row, or a snapshot during a reload, can land
  // two answers out of order; the older one must not win by arriving last.
  let disposed = false;
  let request = 0;
  ...
  async function load() {
    ...
    try {
      const dto = await api.history(islandId, guid, range);
