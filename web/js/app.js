...
// routeSeq numbers the navigations. A view builds asynchronously - it fetches
// before it hands back its unmount - so a second navigation can begin while
// the first is still loading: after a quick double click, and on every page
// load, because route() runs twice at start-up (see the end of this file).
//
// Without this, whichever view finished last took currentUnmount and the
// other one's unmount was lost for good: its listeners went on refetching an
// island nobody was looking at, on every statistics tick, for the rest of the
// session. And a view that failed after being overtaken replaced the page on
// screen with its own error. An overtaken view now unmounts itself as soon as
// it is built, and its failure is dropped.
let routeSeq = 0;
...
async function route() {
  ...
  try {
    switch (r.route) {
      case "help":
        unmount = renderHelp(els.view);
        break;
      case "phone":
        unmount = await renderPhone(els.view, store);
        break;
      case "alerts":
        unmount = await renderAlerts(els.view, store, r.all);
        break;
      case "island":
        unmount = await renderIslandDetail(els.view, r.islandId, store);
        break;
      case "history":
        unmount = await renderHistory(els.view, r.islandId, r.guid, store);
        break;
      case "efficiency":
        unmount = await renderEfficiency(els.view, r.islandId, store);
        break;
      default:
        renderHome(els.view);
    }
  } catch (err) {
    if (seq !== routeSeq) return; // an overtaken view's error is not the page's
