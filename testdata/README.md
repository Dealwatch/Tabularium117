...
Copied verbatim from `anno-mods/anno117-game-connector`, commit
`ed6548f1e706fc8ceafbe1821f82a79292d506de` (2026-09-20), files
`docs/example_responses.txt` and `docs/test-replay-data.jsonl`. That repository
is released into the public domain (Unlicense); the captured game data itself
is © Ubisoft and used here for interoperability testing only.

- `example_responses.txt` – 14 real `AreaProductionStatistics` messages
  (pipe protocol v2) as the connector's SSE lines (`data: {...}`), i.e. the
  decoded JSON form, not raw pipe bytes. Two game sessions, seven islands each,
  all sharing one `timeStamp`.
- `test-replay-data.jsonl` – the connector's replay fixture derived from the
  file above: one line per unique `islandId` (12 lines, so islands whose
  `islandId` repeats across sessions were collapsed), in-game-currency fields
  (`totalIncome`, `totalMaintenance`, `totalProfit`) zeroed out.
...
understood, and a field the connector dropped is missing here too. A real
capture from a running game (task T0.3) will replace it as the primary fixture.
...
A **real recording** from the owner's PC (task T0.3, 2026-09-22, recorded
