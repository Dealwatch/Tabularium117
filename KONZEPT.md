...
4. **Warnungen** – regelbasiert, umgesetzt in `internal/alerts` (Phase 6):
   `deficit` (Delta < 0 in 3 aufeinanderfolgenden Messungen, klärt sich nach 3 Messungen mit
   Delta ≥ 0) und `productivity_drop` (**Effizienz** = `Generation / PerfectGeneration` in Prozent
   fällt um mehr als 20 Prozentpunkte unter ihr eigenes 15-Minuten-Mittel; klärt sich, sobald sie
   wieder innerhalb von 10 Prozentpunkten liegt). Bewusst **nicht** `AverageProductivity`: deren
   Bedeutung ist inzwischen geklärt (§12), aber sie misst den Laufgrad der Gebäude, nicht den
   Ausstoß gegen das Optimum – die Regel bleibt deshalb auf der Effizienz. Die beiden
   unterschiedlichen Schwellen sind die Hysterese: eine Warnung flackert nicht, wenn ein Wert um die Schwelle pendelt. Schwellwerte per
   `--alert-deficit-samples` und `--alert-drop-pp` einstellbar.
...
> **Entschieden (T0.4):** Go. Die Pipe liefert ein flaches Little-Endian-Binärformat mit
...
### SQLite-Schema (Migration 1, umgesetzt in T3.1)
...
### SQLite-Schema (Migration 2, umgesetzt in T6.2)

Die Tabelle `alert` kam bewusst erst in Phase 6 dazu, als ihre Spalten aus der Regel-Engine
...
CREATE TABLE alert (
  ...
  island INTEGER NOT NULL REFERENCES island(id), product_guid INTEGER NOT NULL,
  rule TEXT NOT NULL,                                   -- "deficit" | "productivity_drop"
  severity TEXT NOT NULL,                               -- vorerst immer "warning"
...
### SQLite-Schema (Migration 3, umgesetzt nach T0.3)
...
- Eine **Warnung** ist
  ...
  gespeicherte Warnung, `value` (aktuelles Delta bzw. aktuelle Effizienz) nur für
...
Umgesetzt in Phase 7 (`internal/lan` für Adresswahl und Token,
`internal/server` für Listener, Filter und Endpunkte). Dieser Abschnitt
...
| Pipe liefert weniger als erhofft (z. B. nur aktive Insel) | Features eingeschränkt | Capture zeigt alle Inseln zweier Sessions in einem Tick – Live-Bestätigung in T0.3 (Record-Modus) |
| Spiel erlaubt nur einen Pipe-Client (Konflikt mit Connector) | Tools nicht parallel nutzbar | In T0.3 prüfen; ggf. im README dokumentieren |
...
| Überschneidung mit Connector-Projekt | Doppelarbeit | Vor Phase 2 im anno-mods-Discord abstimmen |
...
## 12. Entscheidungen aus Phase 0 (T0.4)

Grundlage: `docs/protocol.md` (T0.1/T0.2). T0.3 (Praxistest) ist hinter Phase 1 verschoben, damit
mit dem eigenen Record-Modus echte Rohdaten mitgeschnitten werden können statt nur ImGui-Fenster
zu beobachten.
...
- **MVP-Umfang bestätigt.** Die Capture enthält 14 Inseln aus zwei Sessions in einem Tick, also
  ...
  geplant. Bleibt die Live-Bestätigung in T0.3 aus, wird der Scope dort neu geschnitten.
...
- **Aufzeichnung** speichert rohe Frames (§8), damit Fixtures den Decoder beweisen und
  Protokolländerungen überstehen. Die Connector-Fixtures in `testdata/connector/` sind dekodiertes
  JSON und dienen Katalog-Abgleich (T2.4) und UI-Tests.
- **Port 53118** bleibt; der Connector nutzt 53117.

### Live-Befunde 2026-09-22 (T0.3)
